...
class Transitions_terminal(NamedTuple):
    """A batch of obs-act-obs-rew-done transitions.
...
def flatten_trajectories_terminal(trajectories: Sequence[Trajectory]) -> Transitions:
    """Flatten a series of trajectory dictionaries into arrays.
  ...
  """
    ...
    for traj in trajectories:
        ...
        parts["obs"].append(obs[-1:])
        parts["next_obs"].append(obs[-1:])
        dones = np.zeros_like(traj.rews[-1:], dtype=np.bool)
        # dones = np.zeros(1, dtype=np.bool)

        # print("shape of dones is: ", dones.shape)
        dones[-1] = True
        parts["dones"].append(dones)

    # temp = {k: len(v) for k, v in parts.items()}
    # print(temp)
    cat_parts = {
        key: np.concatenate(part_list, axis=0)
        for key, part_list in parts.items()
